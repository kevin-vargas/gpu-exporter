...
import (
	...
	"nvidia_gpu_exporter/internal/discovery"
	"nvidia_gpu_exporter/internal/discovery/consul"
	"nvidia_gpu_exporter/internal/exporter"
	"nvidia_gpu_exporter/internal/ip"
	"nvidia_gpu_exporter/internal/ip/local"
	"nvidia_gpu_exporter/internal/ip/public"
)
...
func main() {
	var (
		...
		discoveryServerAddress = kingpin.Flag("discovery.server", "Address of server for discovery").
					Default("localhost:8500").String()
		typeClient = kingpin.Flag("type", "Type of client").
	...
	http.HandleFunc(health_path, func(w http.ResponseWriter, r *http.Request) {
	...
	var ip ip.IP
	ip = local.New()
	if *typeClient == type_public {
		ip = public.New()
	}
	hostName, err := ip.Get()
	if err != nil {
		_ = level.Error(logger).Log("msg", "Error on get hostname", "err", err)
		os.Exit(1)
	}

	var port string
	for _, p := range *webConfig.WebListenAddresses {
		port = p
	}

	// TODO: add https
	discoveryClientHealthPoint := fmt.Sprintf("http://%s%s%s", hostName, port, health_path)

	d, err = consul.New(*discoveryServerAddress, hostName, discoveryClientHealthPoint, port)

	if err != nil {
		_ = level.Error(logger).Log("msg", "Error on create discovery", "err", err)
		os.Exit(1)
	}
	err = d.Registry()
	if err != nil {
		_ = level.Error(logger).Log("msg", "Error on service discovery", "err", err)
		os.Exit(1)
	}
...
func NewRootHandler(logger log.Logger, metricsPath string) *RootHandler {
	return &RootHandler{
		response: []byte(fmt.Sprintf(redirectPageTemplate, metricsPath, health_path)),
		logger:   logger,
	}
}
